...
private:
    ...
    // Dash 释放前小后撤（蓄力动作）：让冲刺更好读招、更有"起势"
    float dashBackstepDist_      = 0.75f; // 普通冲刺：后撤距离（世界坐标）
    float dashBackstepDistShort_ = 0.45f; // 贴脸小冲刺：后撤距离
    float dashBackstepMaxSpeed_  = 0.11f; // 后撤最大速度（按 60fps 每帧位移）
    float dashBackstepMinSpeed_  = 0.03f; // 后撤最小速度（避免太慢看不出来）
    float dashWindupTotal_       = 0.0f;  // 记录本次 Dash Windup 总时长，用于按时长均匀后撤
    float dashBackstepMoved_     = 0.0f;  // 本次 Windup 已后撤的距离（用于截断）
    // Ultimate（大招）：释放前“起势后撤”
    // 需求：只在大招释放前（Windup 期间）后撤一段距离，然后进入 Ultimate 左右来回冲刺。
    //       大招来回反弹时不再额外后撤。
    float ultimateWindupBackstepDist_      = 2.00f; // 大招释放前：后撤距离（比普通更长）
    float ultimateWindupBackstepMaxSpeed_  = 0.14f; // Windup 后撤最大速度（按 60fps 每帧位移）
    float ultimateWindupBackstepMinSpeed_  = 0.04f; // Windup 后撤最小速度
    float ultimateWindupTotal_             = 0.0f;  // 记录本次大招 Windup 总时长
    float ultimateWindupBackstepMoved_     = 0.0f;  // Windup 已后撤距离（用于截断）
