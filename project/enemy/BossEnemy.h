...
private:
    ...
    uint32_t battleTriggerXIndex_ = 33;
    ...
    float queuedDashSpeed_ = 0.45f;  // Dash 実際に使う速度
    ...
    // ---------- 调参区 ----------
    float moveSpeed_ = 0.18f;  // 追跡速度（「1フレーム移動量」）
    ...
    float closeDashSpeed_ = 0.30f;  // 小ダッシュ速度（1フレーム移動量）
