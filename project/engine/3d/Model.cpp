...
ModelData Model::LoadObjectFile(const std::string& directoryPath, const std::string& filename)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (identifier == "v") {
            ...
            // ツール: 空要素を保持して "v/vt/vn"、"v//vn"、"v/vt" を分割する
            ...
            auto parseIndex = [](const std::string& s, int count) -> int {
                ...
                if (idx > 0) return idx - 1;         // OBJ の正のインデックスは 1 始まり
            ...
            for (auto& t : tokensF) {
                auto parts = splitKeepEmpty(t);                // 空要素を保持する
                int vi = parseIndex(parts[0], (int)positions.size());
                int vti = parseIndex(parts[1], (int)texcoords.size()); // 空の場合は -1
                int vni = parseIndex(parts[2], (int)normals.size());   // 空の場合は -1
            ...
            // 三角ファンとして分割し、従来と同じ逆順 push で頂点順を維持する
            for (size_t i = 1; i + 1 < vIdx.size(); ++i) {
                ...
                for (int k = 0; k < 3; ++k) {
                    ...
                    // 座標系は v 読み込み時に x *= -1 済みのため、ここでは同じ向きを維持する。
